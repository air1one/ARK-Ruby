...
describe Ark::TransactionBuilder do
  ...
  shared_examples_for 'a transaction' do
    it 'is valid' do
      expect(Ark::Util::Crypto.verify(subject)).to be_truthy
    end
  end

  shared_examples_for 'a transaction signed twice' do
    let(:second_public_key_address) { Ark::Util::Crypto.get_key(second_secret).public_key.unpack('H*').first }

    it 'is valid' do
      expect(Ark::Util::Crypto.verify(subject)).to be_truthy
      expect(Ark::Util::Crypto.second_verify(subject, second_public_key_address)).to be_truthy
    end
  end
